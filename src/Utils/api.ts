...
const _isPerChannelUrl = (url: string): boolean =>
  /\/channels\/[0-9a-f]{32}\//i.test(url);

const _parseAndValidateCache = (
  cachedDataString: string | null,
  expiryMs: number
): any | null => {
...
// 공통 fetch 함수 (캐시 포함)
// ===========================
export const fetchBroadList = async (
  url: string,
  expiry_seconds = 50,
  timeout = 5000
): Promise<any> => {
        ...
        try {
          if (response.status >= 200 && response.status < 300) {
            ...
            if (
              ...
              (jsonResponse?.code && jsonResponse.code < 0)
            ) {
              customLog.error(
                url,
...
export const getHiddenbjList = async (): Promise<string[]> => {
  const url =
    "https://live.sooplive.com/api/hiddenbj/hiddenbjController.php";
...
export const getStationFeed = async (
  isChannelFeedEnabled: boolean
): Promise<any[]> => {
  if (!isChannelFeedEnabled) return [];

  const feedUrl =
    "https://myapi.sooplive.com/api/feed?index_reg_date=0&user_id=&is_bj_write=1&feed_type=&page=1";
    ...
    GM_xmlhttpRequest({
      ...
      onload: function (response) {
        if (response.status === 200) {
          ...
          szBroadCategory = JSON.parse(
            szBroadCategory.split("var szBroadCategory = ")[1].slice(0, -1)
          );
          if (szBroadCategory.CHANNEL.RESULT === "1") {
            ...
            const cleanData: I_CategoryData = {
              CHANNEL: {
                ...
                BROAD_CATEGORY: stripCategoryKeys(
                  szBroadCategory.CHANNEL.BROAD_CATEGORY
                ),
              },
            };
...
export const getBroadM3u8Domain = async (
