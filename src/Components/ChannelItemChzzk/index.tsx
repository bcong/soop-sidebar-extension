...
const ChannelItemChzzk: React.FC<ChannelItemChzzkProps> = observer(
  ({ data }) => {
    ...
    const channelName =
      channel.channel?.channelName ?? channel.channelName ?? channelId;
    const liveTitle =
      channel.liveInfo?.liveTitle ?? channel.liveTitle ?? "";
    const viewers =
      channel.liveInfo?.concurrentUserCount ?? channel.concurrentUserCount ?? 0;
    ...
    const handlePinClick = (e: React.MouseEvent) => {
      ...
      if (isPinned) {
        sidebarStore.setPinnedChzzkUsers(
          sidebarStore.pinnedChzzkUsers.filter((id) => id !== channelId)
        );
      } else {
        sidebarStore.setPinnedChzzkUsers([
    ...
    const handleContextMenu = (e: React.MouseEvent) => {
      e.preventDefault();
      if (
        confirm(`"${channelName}" (${channelId}) 를 차단하시겠습니까?`)
      ) {
        blockUser(
          channelId,
          channelName,
    ...
    return (
      ...
      >
        ...
        <span className="description">{liveTitle}</span>
